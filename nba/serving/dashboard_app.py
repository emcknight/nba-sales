...
min_ev, max_ev = st.sidebar.slider(
    ...
    (float(df["expected_value"].quantile(0.05)), float(df["expected_value"].quantile(0.95)))
)
...
ev_by_action = filtered_df.groupby("action_type")["expected_value"].sum().sort_values(ascending=False)
...
# Check if the column exists; skip gracefully if not
if "segment" in df.columns:
    ...
    pivot = seg_summary.pivot(index="segment", columns="action_type", values="total_ev").fillna(0)
...
st.dataframe(top_accounts[["account_id", "action_type", "expected_value", "uplift", "acv"]])
